...
#Create dataset
dataset = torch.load('dataset/het_graph_paper_mesh_splitted_eval.pk')
data = dataset
...
outdir = 'out_embeddings_test_perf'
os.makedirs(outdir, exist_ok = True)

best_val_perf = test_perf = 0
best_val_pp = best_val_pm = 0
for epoch in range(1, 100):
	...
	if val_perf > best_val_perf:
		...
		best_val_pp = val_perf_pp
		best_val_pm = val_perf_pm
	...
	log = 'Epoch: {:03d}, Loss: {:.4f}, Train ROCAUC: {:.4f}, Val ROCAUC: {:.4f}, paper-paper: {:.4f}, paper-mesh: {:.4f}, Test ROCAUC: {:.4f}, paper-paper: {:.4f}, paper-mesh: {:.4f}'
...
print('Best val ROCAUC: {:.4f}, paper-paper: {:.4f}, paper-mesh: {:.4f}'.format(best_val_perf, best_val_pp, val_perf_pm))
print('Best test ROCAUC: {:.4f}'.format(test_perf))
